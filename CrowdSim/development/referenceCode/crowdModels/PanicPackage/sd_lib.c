...
// MOD: Changed the output files to csv files with a more descriptive name.
char *OFN_DEFAULT = "panicPackageData.csv";
char *OF2N_DEFAULT = "panicPackageData2.csv";
...
void Upd(){
  ...
  */  
  ...
  /* 1.1 */
  /* wall force */
  /* Calculate forces for each individual */
  for(i=0;i<N;i++){
...
void Init_Demo(){
  ...
  // MOD: added tstep to file header
  fprintf(OFP,"UpdNum,SimTime,N,<E>,tStep\n");
  fflush(OFP);
...
void Save_Demo(){
  ...
  if(  (UpdNum==0)
	...
	// MOD: Added printing out current Euler Time Step
	tstep = SimTime[UpdNum] - SimTime[UpdNum-1];
	fprintf(OFP,"%d,%g,%d,%g,%g\n",UpdNum,SimTime[UpdNum],N,e_mean,tstep);
	fflush(OFP); 
